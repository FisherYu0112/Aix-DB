...
- **此技能是指令文档，不是工具** - 不要说"调用报告生成技能"然后停下，直接按以下工作流程执行到底：查询数据 → 生成 HTML → 用分隔符包裹输出
...
**重要：** 生成 HTML 时，直接应用以下 UI/UX Pro Max 设计原则以获得专业、美观的输出。
...
**⚠️ 注意：直接在你的 HTML 输出中应用这些设计原则，不需要调用任何子代理或额外工具。**
...
- **生成 HTML 时，直接应用 UI/UX Pro Max 设计原则**（专业配色、卡片布局、响应式设计）
